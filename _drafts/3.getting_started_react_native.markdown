...
After installation. To start directly with an expo app use `expo init projectName` then do `expo start`. You can also clone the [**source code repository**](https://github.com/fbohz/react-native-blogs-practice/tree/master/1.Demo). Then just do `npm install` and `expo start`.

*Note:* You'll need to install the mobile app version (either in AppStore or GooglePlay) of Expo for your particular device and create an account. Once there just scan from your app the generated QR code.
...
As you follow along you can download and checkout the following repo [here](https://github.com/fbohz/react-native-blogs-practice/tree/master/1.Demo). If you navigate to `App.js`, you'll notice `StyleSheet.create`, this is optional and you can also pass styles **inline** just like in React.
...
To install better read documentation [here](https://reactnavigation.org/docs/getting-started/).
...
We add the text as string and use the `onPress` attribute. Remember Button is self-closed. Out of both for navigation we recommend TouchableOpacity element if you need more customization.
...
With props passsed by the stackNavigator we can tell what to show or not on the screen. To access the navigation object you access it by `props.navigation`. On the included `props.navigation.navigate()` method we can pass in the screen we defined on App.js and it will render the component we want! As easy as:

`onPress={() => props.navigation.navigate('ComponentName')}`

Then this takes us to the basic navigation and UI display with React Native!
